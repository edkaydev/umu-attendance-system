  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="flex gap-2 border-b border-border">
        {(['session', 'access', 'danger'] as const).map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`border-b-2 px-4 py-2 text-body-sm font-semibold capitalize ${
              activeTab === tab
                ? tab === 'danger' ? 'border-danger text-danger' : 'border-umu-red text-umu-red'
                : 'border-transparent text-text-secondary'
            }`}
          >
            {tab === 'session' ? 'Global Session' : tab === 'access' ? 'Account Settings' : 'Danger Zone'}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'access' && (
        <Card title="Profile Editing Access" noPadding>
          {PROFILE_SCOPES.map(({ scope, label, description }) => {
            ...
            return (
              <div key={scope} className="flex items-center justify-between gap-4 border-b border-border px-5 py-4 last:border-b-0">
                <div className="min-w-0">
                  <p className="font-semibold text-text-primary">{label}</p>
                  <p className="text-body-sm text-text-secondary">{description}</p>
                </div>
                <button
                  onClick={() => toggleProfile(scope)}
                  disabled={!profileEditing || saving !== null}
                  aria-label={`Toggle ${label} profile editing`}
                  aria-pressed={enabled}
                  className={`relative inline-flex h-7 w-12 shrink-0 items-center rounded-full ${enabled ? 'bg-success' : 'bg-text-disabled'} disabled:opacity-60`}
                >
        ...
        <Card>
          <div className="mb-6 flex items-start gap-3 rounded border border-danger-border bg-danger-light p-4">
            ...
            <div>
              ...
              <p className="mt-1 text-body-sm text-text-secondary">These operations permanently delete data. Use only at the end of an academic semester.</p>
            </div>
          </div>
          ...
          <div className="mb-4 flex flex-col gap-2 rounded border border-border bg-surface-0 p-5 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 max-w-lg text-body-sm text-text-secondary">Clears browser-cached assets. No data deleted.</p>
            </div>
            <Button variant="secondary" className="mt-3 shrink-0 sm:mt-0" loading={clearingCache} onClick={handleClearCache}>Clear Cache</Button>
          </div>
          ...
          <div className="flex flex-col gap-2 rounded border border-danger-border bg-surface-0 p-5 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 max-w-lg text-body-sm text-text-secondary">
                Permanently deletes all students, lecturers, faculty admins, sessions, attendance records,
                enrollments, assignments, faculties, programmes, course units, and curriculum. System Admin accounts and settings are kept.
              </p>
            </div>
            <Button variant="danger" className="mt-3 shrink-0 sm:mt-0" onClick={() => { setResetConfirmText(''); setShowResetModal(true) }}>Reset Database</Button>
          </div>
        </Card>
      )}

      <Modal open={showResetModal} onClose={() => !resetting && setShowResetModal(false)} title="Reset Database">
        <div className="space-y-5">
          <div className="flex items-start gap-3 rounded border border-danger-border bg-danger-light p-4">
            <div className="text-body-sm text-text-primary">
              <p className="font-semibold text-danger">This will permanently delete all academic data.</p>
          ...
          <div className="flex justify-end gap-3 pt-1">
            ...
            <Button variant="danger" loading={resetting} disabled={resetConfirmText !== 'RESET'} onClick={handleReset}>Reset Everything</Button>
          </div>
        </div>
      </Modal>
    </div>
