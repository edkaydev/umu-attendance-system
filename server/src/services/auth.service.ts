...
export function mapOAuthError(message: string): string {
  switch (message) {
    case 'INVALID_DOMAIN':    return 'invalid-domain'
    case 'NOT_REGISTERED':    return 'not-registered'
    case 'ACCOUNT_DISABLED':  return 'account-disabled'
    case 'No email returned from Google': return 'no-email'
...
export async function getCurrentUser(userId: string) {
  const user = await prisma.user.findUnique({
    ...
    select: {
      ...
      additionalFaculties: {
        select: { faculty: { select: { id: true, name: true, code: true } } },
      },
      programmeId:     true,
  ...
  return {
    ...user,
    additionalFaculties: user.additionalFaculties.map((uf) => uf.faculty),
  }
}
